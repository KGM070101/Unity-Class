...
## 상속
상속은 **기존 클래스의 기능을 재사용, 확장, 수정**하면서 새로운 클래스를 만드는 것을 뜻한다.  
**상속받는 클래스를 자식 클래스(derived class), 물려주는 클래스를 부모 클래스(base class)라고 부른다.**

---

## 다형성
다형성은 **실행 시점에 자식 클래스의 객체를 부모 클래스 타입으로 다룰 수 있다**.  
